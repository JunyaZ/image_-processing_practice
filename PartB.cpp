...
int main()
{
	...
	ofstream os ("Assn2PartB_Junya009.bmp", ofstream::binary);
	...
	if (is)   // if file was opened correctly . . .
	{
	    ...
	    height += (buffer[dataIndex + 7] & 0xff)  << 24;   // This is the 4nd-leastSigByte ie MostSigByte
		...
		for (int i = 0; i < height; i++) // Number of rows of BMP image
		{

			// Set dataCounter, which is an index into the buffer at the start byte of each row
			dataCounter = 54 + (i * width * 3);

			// Manipulate each pixel on each row of the BMP image
			for (int j = 0; j < (width*3 ); j += 3) // Number of pixels on each row
			{
					tempC = buffer[dataCounter + j + 0];
					os.write (tempCPtr, 1);  // read data as a block or group (not individually)
					os.write (tempCPtr, 1);  // read data as a block or group (not individually)
					os.write (tempCPtr, 1);  // read data as a block or group (not individually)
