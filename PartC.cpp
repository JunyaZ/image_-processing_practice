...
int main()
{
	...
	ofstream os ("Assn2PartC_Junya009.bmp", ofstream::binary);
	...
	if (is)   // if file was opened correctly . . .
	{
        ...
        {
                int change_X = width* 3 - j  ;
                tempC = buffer[dataCounter+ (change_X+0) ];
                os.write(tempCPtr,1);
                tempC = buffer [dataCounter+ (change_X+1) ];
                os.write(tempCPtr,1);
                tempC = buffer [dataCounter+ (change_X+2) ];
                os.write(tempCPtr,1);
